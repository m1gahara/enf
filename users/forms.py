...
class CustomUserUpdateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not clean_email.get("email"):
            cleaned_data["email"] = self.instance.email
        for fields in ["company", "address1", "address2", "city", "country",
                       "province", "postal_code", "phone"]:
            if cleaned_data.get(fields):
                cleaned_data[fields] = strip_tags(cleaned_data[fields])
        return cleaned_data        