...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            login(request, user, backend="django.cotrib.auth.backends.ModelBackend")
...
@login_required(lodin_url="/users/login")
...
def account_details(request):
    ...
    return TemplateResponse(request, "users/partials/account_details.html", {"user"})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("HX-Request"):
                return TemplateResponse(request, "users/partials/account_details.htlm", {"user": updated_user})
            return TemplateResponse(request, "users/partials/account_details.html", {"user": updated_user})
        else:
            return TemplateResponse(request, "users/partials/edit_account_details.htlm", {"user": request.user, "form": form})
    if request.headers.get("HX-Request"):
        return HttpResponse(headers={"HX-Redirect": reverse("users:profile")})
    return redirect("users:profile")
